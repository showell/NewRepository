...
REPO = "/home/steve/showell_repos/NewRepository"
ACCEL = __import__("os").environ.get("CODEX_ACCEL", "tcg")
BASE_PORT = 56400

def launch(kernel, mem_mb=3072):
    data_port, ctrl_port = BASE_PORT, BASE_PORT + 1
...
def run_cdx(kernel, mem_mb=1024, timeout=300):
    proc, data, ctrl = launch(kernel, mem_mb)
    try:
        t0 = time.time()
        out = recv_all(data, idle_timeout=60, overall_timeout=timeout)
